    ...
    return (
        <div className="flex flex-col lg:flex-row md:flex-row items-center">
            {priceDetails.map((details, index) => {
                ...
                return (
                    < div key={index} className="bg-black w-[50%] text-center my-8 lg:mx-2 shadow-md border-t border-emerald-100 shadow-emerald-900 
                rounded-xl hover:scale-105 transition-transform duration-300 ease-out" >
                        <h1 className="text-emerald-300 absolute bg-emerald-800 -top-7 py-3 px-6 left-1/2 -translate-x-1/2 capitalize rounded-3xl">{details.popularity}</h1>
                        ...
                        <button className="py-3 m-5 bg-emerald-700 text-gray-200 px-4 capitalize hover:text-emerald-800 hover:bg-white
                   rounded-xl transition-colors duration-300 ease-in "><a href="#">purchase now</a></button>
                    </div>
